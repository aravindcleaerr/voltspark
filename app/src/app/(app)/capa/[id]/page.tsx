    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {capa.autoGenerated && capa.incident && (
            <div className="card bg-purple-50 border-purple-200">
              <p className="text-xs text-purple-700 font-medium mb-1">Auto-generated from incident</p>
              <Link href={`/safety/incidents/${capa.incident.id}`} className="text-sm text-purple-900 hover:underline">
                {capa.incident.title} →
              </Link>
              <p className="text-xs text-purple-600 mt-1">Severity: {capa.incident.severity} · Status: {capa.incident.status.replace('_', ' ')}</p>
            </div>
          )}

          {capa.frameworkRequirement && (
            <div className="card bg-blue-50 border-blue-200">
              <p className="text-xs text-blue-700 font-medium mb-1">Linked compliance requirement</p>
              <p className="text-sm text-blue-900">
                <span className="font-medium">{capa.frameworkRequirement.framework.code}</span> · {capa.frameworkRequirement.code}
              </p>
              <p className="text-xs text-blue-700 mt-0.5">{capa.frameworkRequirement.title}</p>
            </div>
          )}
        </div>
